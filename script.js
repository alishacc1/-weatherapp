let weatherAPIKey = "d7b6c8559f9281ca7d0e3e6ae67cf3ff";
...
window.onload = function show() {
  let option = { enableHighFrequency: true, timeout: 5000, maximumAge: 0 };
 ...
 async function success(pos) {
  ...
  let weath=await abst();
  console.log(weath.city.name);
  weatherForCity(weath.city.name);
...
function error(err) {
  ...
  let abst = async () => {
    // let endPoint = ;
    let response = await fetch(currentLocation);
    let weather = await response.json();
    // console.log(weather);
    return weather;
  };